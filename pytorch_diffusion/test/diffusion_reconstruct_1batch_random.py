...
from pytorch_diffusion.test.diffusion_reconstruct_1batch import *
...
class DiffusionReconstruct1BatchRandomInit(DiffusionReconstruct1Batch):
    def __init__(self, diffusion_config, model_config, device=None, train=True,
        ...
        self.optimizer_difmodel = optim.SGD(self.model.parameters(), lr=lr, weight_decay=weight_decay)
    ...
    def training(self, n, number_of_iters=100):
        ...
        for j, batch in enumerate(self.data_loader, 0):
            ...
            for i in range(number_of_iters):
                ...
                self.optimizer_difmodel.zero_grad()
                final_loss = loss_iter
                final_loss.backward()
                self.optimizer.step()
                self.optimizer_difmodel.step()

                self.tensorboard_writer.add_scalar("Loss/train_loss_iter", loss_iter.item(), i_iter)

                if i_iter % 100  == 0 or i == number_of_iters - 1:
