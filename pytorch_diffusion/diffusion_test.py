...
from pytorch_diffusion.model import Model, ModelRNN
...
class DiffusionRNN(Diffusion):
    def __init__(self, diffusion_config, model_config, device=None):
        self.init_diffusion_parameters(**diffusion_config)
        self.model = ModelRNN(**model_config)
        if device is None:
            device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
        self.device = device
        self.model.to(self.device)
...
if __name__ == "__main__":
    import sys
    from tqdm.notebook import tqdm as tqdma
    ...
    for ib in tqdma(range(nb), desc="Batch"):
        x = diffusion.denoise(bs, progress_bar=tqdma)
        ...
        exit(0)
    # l = [module for module in diffusion.model.modules() if not isinstance(module, torch.nn.Sequential)]
    # print(l)